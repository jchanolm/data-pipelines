...
class ManualIngestor(Ingestor):
    def __init__(self):        
        ...
        self.grants_df = pd.read_csv(f"{self.filepath}/data_grants_manual_20240307.csv")
    ...
    def process_wallets(self):
        wallets_df_direct = self.grantees_direct_df[self.grantees_direct_df['fundingWallet'].notna() 
        & (self.grantees_direct_df['fundingWallet'] != '')]
    ...
    def process_twitter(self):
        ...
        & (self.grantees_direct_df['twitterHandle'] != '')]

        twitter_df_ecosystem = self.grantees_direct_df[self.grantees_ecosystem_df['twitterHandle'].notna() 
        & (self.grantees_ecosystem_df['twitterHandle'] != '')]
    ...
    def process_github(self):
        ...
        & (self.grantees_direct_df['githubHandle'] != '')]
        github_df_ecosystem = self.grantees_direct_df[self.grantees_ecosystem_df['githubHandle'].notna() 
        & (self.grantees_ecosystem_df['githubHandle'] != '')]
    ...
    def process_dune(self):
        dune_df_direct = self.grantees_direct_df[self.self.grantees_direct_df['duneHandle'].notna() 
        & (self.grantees_direct_df['duneHandle'] != '')]
        dune_df_ecosystem = self.grantees_direct_df[self.grantees_ecosystem_df['duneHandle'].notna() 
        & (self.grantees_ecosystem_df['duneHandle'] != '')]

        dune_direct_urls = self.save_df_as_csv(dune_df_direct, f"data_dune_direct_{self.asOf}.csv")
        dune_ecosystem_urls = self.save_df_as_csv(dune_df_ecosystem, f"data_dune_df_ecosystem_urls_{self.asOf}.csv")

        self.cyphers.create_dunes(dune_direct_urls)
        self.cyphers.create_dunes(dune_ecosystem_urls)

    def process_websites(self):
        websites_df_direct = self.grantees_direct_df[self.grantees_direct_df['websiteUrl'].notna() 
        & (self.grantees_direct_df['websiteUrl'] != '')]
        websites_df_ecosystem = self.grantees_direct_df[self.grantees_ecosystem_df['websiteUrl'].notna() 
        & (self.grantees_ecosystem_df['websiteUrl'] != '')]
        ...
        self.cyphers.create_githubs(website_df_direct_urls)
        self.cyphers.create_githubs(website_df_ecosystem_urls)

    def process_grants(self):
        grants_initiatives_df = self.grantees_direct_df[['grantInitiative', 'grantsPlatform', 'grantTokenAddress']]
        deduplicated_grants_df = grants_initiatives_df.drop_duplicates()
        grants_urls = self.save_df_as_csv(deduplicated_grants_df, f'data_grants_{self.asOf}.csv')
        self.cyphers.create_grants(grants_urls)

    def process_funders(self):   
        self.cyphers.conenct_funders_grants()
    ...
    def process_submission_grants(self):
        self.connect_submission_grants()
